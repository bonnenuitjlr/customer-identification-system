...
public class RedisConfiguration {
	...
	public RedisConnectionFactory connectionFactory(String hostName, int maxIdle, int maxTotal, int database,
			long maxWaitMillis, boolean testOnBorrow, String mode, String master4Sentinel) throws Exception {
		...
		switch (_mode) {
			case stand_alone: {
				return connectionFactoryStandAlone(hostName, maxIdle, maxTotal, database, maxWaitMillis,testOnBorrow);
			}
			case cluster: {
				return connectionFactoryCluster(this.getNodes(hostName,mode), maxTotal, maxIdle, database, maxWaitMillis,testOnBorrow);
			}
			case sentinel: {
				return connectionFactorySentinel(this.getNodes(hostName,mode), master4Sentinel, maxTotal, maxIdle, database,maxWaitMillis, testOnBorrow);
			}
			default: {
	...
	public RedisConnectionFactory connectionFactoryStandAlone(String hostName, int maxIdle, int maxTotal,
			int index, long maxWaitMillis, boolean testOnBorrow) {
	...
	private RedisConnectionFactory connectionFactorySentinel(Set<String> nodes, String master4Sentinel, int maxTotal,
			int maxIdle, int database, long maxWaitMillis, boolean testOnBorrow) {
	...
	private RedisConnectionFactory connectionFactoryCluster(Set<String> nodes, int maxTotal, int maxIdle, int database,long maxWaitMillis, boolean testOnBorrow) {
